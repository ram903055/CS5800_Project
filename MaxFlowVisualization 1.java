        ...
        for (int i = 0; i < graph.V; i++) {
            ...
            for (Edge e : graph.adj.get(i)) {
                if (e.capacity > 0) {  // Only draw forward edges
                    ...
                    String label = e.flow + "/" + e.capacity;
...
public class MaxFlowVisualization {
    private static final String[] VERTEX_NAMES = {"Source", "A", "B", "C", "D", "Sink"};

    public static void main(String[] args) {
        Graph graph = createGraph();
        int maxFlow = MaxFlowAlgorithms.fordFulkerson(graph, 0, 5);

        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("Max Flow Visualization");
            ...
            JOptionPane.showMessageDialog(frame, "Maximum Flow: " + maxFlow);
        });
    }
